...
Used for resources whose consumer expects a *folder per item* on
disk (Claude skills are the canonical example: each skill is a
directory containing ``SKILL.md`` plus assets). A source directory
of only-subdirectories expands to one bundle per subfolder; a
source directory with top-level files becomes a single bundle.
"""
